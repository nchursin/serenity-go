...
check_git_cliff() {
    if [[ ! -f "$HOME/bin/git-cliff" ]]; then
        echo -e "${YELLOW}⚠️  git-cliff not found. Using basic git log for changelog.${NC}"
        return 1
    fi
    return 0
}
    ...
    if [[ "$LAST_TAG" == "" ]]; then
        ...
        if [[ $COMMITS_SINCE_TAG -gt 0 ]]; then
            if check_git_cliff; then
                ...
                git log --format="%h %s" "$LAST_TAG..HEAD" | head -20
            fi
        else
...
case "$COMMAND" in
    ...
    "prepare")
        ...
        if check_git_cliff; then
            ...
            LAST_TAG=$(git describe --tags --abbrev=0 2>/dev/null || echo "")
            if [[ "$LAST_TAG" == "" ]]; then
                git log --format="- %s" -20 >> CHANGELOG.md
            else
                git log --format="- %s" "$LAST_TAG..HEAD" >> CHANGELOG.md
            fi
        fi
